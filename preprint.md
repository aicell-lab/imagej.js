...
**Box 2 | What ImageJ.JS is not.** The regime argument of §8 can also read abstractly. The three counter-vignettes below name three regimes where ImageJ.JS is the wrong tool on purpose — a connectomics reconstruction, a high-content drug screen, an emergency-department triage — and point at the tools that are right. No new claim is introduced; every method named is defined in §8, and the composition surface through which these tools reach an ImageJ.JS session (Fig 7) is already drawn.

**The 50-terabyte electron-microscopy volume** (→ §8 ¶2, Fig 7). A connectomics laboratory has acquired a 50 TB serial-section EM volume of a mouse brain sample and must trace every neurite through it. The goal is a connectivity matrix, not a figure. Segmenting even a single section by hand is a PhD's worth of work; segmenting the stack by human hand is impossible. The right tool here is a trained convolutional-backbone segmenter — Cellpose [Stringer & Pachitariu 2025] for cell-body recognition, or a domain-tuned flood-fill network for neurite tracing — run on a GPU cluster, not a browser. ImageJ.JS's client-side, human-in-the-loop design is a feature in the small-data regime and a bug here. The right workflow opens the volume with a server-side deep-learning pipeline; ImageJ.JS enters, if at all, only at the figure-preparation stage, on a 200-megapixel region an expert biologist wants to inspect by eye before it goes to the reviewer.
...
**The Tuesday-night emergency-department triage** (→ §8 ¶4, Fig 7). An emergency-department clinician needs a binary "further investigation or discharge" decision on a cardiac ultrasound in under 30 seconds; the patient is in the next room. The right tool here is a calibrated, regulatorily-cleared deep-learning classifier with known sensitivity and specificity, called from the hospital's EMR, not a browser-based exploratory analysis platform. ImageJ.JS's strengths — you can see what the macro did, you can change a threshold and re-run, you can share the URL with a colleague — are liabilities at 02:00 when the clinician has 30 seconds and a policy requirement to log the decision under an FDA-cleared device. ImageJ.JS composes with such a classifier (Fig. 7; Hypha-RPC and MCP surfaces of `hypha-imagej-service.js`) for retrospective case review and teaching, but is explicitly not the instrument that makes the triage call. The regime boundary is a boundary of clinical workflow, not of capability.
...
## Drafted prose — §9 Discussion implications (v0.2, biologist-voice rewrite 2026-04-18, iter 37)

*Sixth application of the iter-23 biologist-voice copy-edit iteration kind, and a notable one — the first to challenge a previously-asserted "this surface is genre-constrained and not subject to biologist-voice" claim. §9 had remained in editorial methodology-research voice through 36 iterations on the assumption that a Discussion section addressing methodology review was inherently genre-constrained. Iter 37 tests and reverses that assumption while preserving the methodology-research argument verbatim. All five paragraphs re-voiced to lead concrete-before-abstract; five new `<h3>` subsection headings match the §3/§4/§8 inline-heading idiom for the biologist reader's navigation; paragraph-level cross-references back to §§1–8 + §10 added as `href` targets. Every claim, citation (Ouyang in prep), italicised concept (`regime fit`, `regime correctness`, "regime, not ranking"), and placeholder (`[N]`, `[48]%` ×2, `[20]%`) of v0.1 preserved verbatim — claim-diff against v0.1 empty, `placeholder-value` inventory unchanged at 185 spans (sixteenth consecutive empty-claim-diff iteration). Biologist-voice chain now reads Key Points → Abstract v0.6 → §1 v0.2 → Box 1 → §2 v0.2 → Fig 1 → Box 2 → §3 v0.2 → Fig 2 → §4 v0.2 → Fig 3 → §§5–7 → Fig 4–6 → §8 v0.2 → Fig 7 → **§9 v0.2** → Cover letter v0.2 → Research Briefing v0.2 — every author-voiced argumentative surface of the paper now in publication-ready biologist voice; only §10 Availability remains in fixed-format editorial voice (the genre constraint that the Availability statement is a list of artefacts, not an argument addressed to the biologist).*

### What the preceding sections add up to

A biologist who reads this paper from §1 to §8 sees four kinds of evidence stacked on top of each other. §2 measures the regime the majority of biology actually occupies — small *N*, human-in-the-loop, long-tail imagery — and shows that it is large and well-defined, not a residual category. §§3 and 4 show that an unmodified classical toolchain can be delivered through a browser tab at a correctness and reproducibility standard comparable to, and in some respects exceeding, what the same biologist gets from desktop Fiji today. §§5–7 show that the same tool reaches places the methodology mainstream has not reached — fifteen undergraduates on Chromebooks in a cell-biology lab, a pathologist working an unusual case-series on an air-gapped hospital workstation, two researchers on different continents measuring the same image together in real time. §8 draws the regime's outer boundary and names the composition surface with the methods that work beyond it. The claim that emerges from these four stacked together is not about any one capability. It is that the bioimage-methodology literature has implicitly treated "tooling for the small-data, human-centred majority" as a solved problem, and that the cost of that assumption has been paid by the readers of that literature rather than by its authors.

### Two missing words for the methodology reviewer

What follows is procedural, not ideological. A reviewer scoring a tool paper today is asked two questions: is the core algorithm new, and does it beat the previous best on a standard benchmark? For the biologist of §2, neither question is the right one. The classical algorithms a small-data biologist actually uses — thresholding, watershed, connected components, careful manual ROI work — are two decades old, well understood, and emphatically not in need of replacement; and no benchmark assembled from large, in-distribution image collections will detect whether a tool can survive a Chromebook teaching lab, a hospital firewall, or a cross-continent consult. Two words are missing from the methodology reviewer's vocabulary. *Regime fit*: does this tool work for a well-defined, empirically large class of users the methodology community has not yet measured? *Regime correctness*: is the tool's behaviour verifiable, reproducible, and auditable under the constraints that class of users actually faces? We do not propose that algorithmic-novelty papers should be displaced. We propose that regime-serving tool papers should be accepted as a first-class category alongside them, scored against the questions their regime actually raises — and that a journal of record for bioimage methodology is the natural place for that category to live.

### What the three design decisions look like as scoring criteria

Three of the design decisions in ImageJ.JS are re-readable as concrete proposals for what such scoring criteria could look like. *Continuity with an existing substrate* (§3) is a regime-fit decision: a new tool that fragments the substrate the biologist majority already uses — the menus, the macros, the plugins — makes the regime harder to reach, no matter how elegant the new tool is on its own. *Zero install as correctness* (§3) is a regime-correctness decision: any tool whose deployment depends on administrative privileges the target biologist does not hold is incorrect for that biologist, and should be scored accordingly in review rather than waved through as an "ergonomics issue". *URL-addressable state plus a pinned-runtime Hypha-RPC verifier* (§§3, 4, 10) is a reproducibility-under-regime decision: it treats reproducibility as a property a reviewer can re-execute at continuous-integration cadence, not as a prose promise hidden in a Methods section. None of these three decisions is itself algorithmically novel; each is a load-bearing commitment the tool must keep to serve its regime, and each is the kind of decision a reviewer's current vocabulary makes hard to praise or to fault.

### The agentic regime needs a healthy human-centred substrate

A second implication concerns the boundary between human-centred and agent-driven image analysis introduced in §8. A bench biologist watching the agentic-bioimage literature emerge could be forgiven for thinking that human-driven tools and agent-driven ones are competing for the same future. The reverse is closer to the truth: the agentic methods stand to benefit most from a well-served human-centred substrate beneath them. An agent that composes ImageJ.JS through the Hypha-RPC and Model Context Protocol interfaces of §10 inherits a verifiable, user-auditable execution environment, a client-side data-governance posture (the patient's slide pixels do not leave the patient's hospital), and a decade-of-Fiji macro corpus as its tool library. A companion paper (Ouyang et al., in preparation) develops this direction. The point for the present Discussion is that first-class tooling for the small-data, human-centred regime is not a retreat from agentic bioimage analysis but a prerequisite for the kind of agentic bioimage analysis that takes its data-governance and reproducibility responsibilities seriously. Here too, "regime, not ranking" is the right frame.

### The measurement instruments are the deeper contribution
...
## Drafted prose — §10 Availability (v0.2, biologist-voice rewrite 2026-04-18, iter 40)

*Seventh application of the iter-23 biologist-voice copy-edit iteration kind and a direct continuation of the iter-37 inverse-genre-constraint pattern. §10 Availability had remained in editorial voice through iters 8–39 on the chain-of-voice assumption that a fixed-format list of artefacts addressed to a copy-editor is inherently genre-constrained. The iter-37 test applied here: re-voicing preserves every URL, file name, licence, placeholder, method name, and DOI verbatim, so §10 can in fact be re-voiced for the biologist reader without losing its copy-editor readability. The biologist-voice rewrite opens each paragraph by addressing the biologist reader directly ("You can open ImageJ.JS…", "Three kinds of data sit under the empirical claims…", "If you want to call ImageJ.JS from a script…", "On telemetry: we record only…"); sentence-level ordering moves concrete-before-abstract; four `<h3>` subsection headings are added following the §9 v0.2 idiom so that a biologist skim-reader can navigate the section without reading every paragraph end-to-end. Every `[DAU]`, `[YYYY]`, `[DOI]`, `[48]%`, `[20]%`, `[X]`, `[Y]/30`, `[Z]/30`, `[N]` placeholder is preserved verbatim; every `https://ij.aicell.io`, `https://github.com/aicell-lab/imagej.js`, `index.html`, `hypha-imagej-service.js`, `collab/`, `v1.0-paper`, `main`, `survey_schema.md`, `survey_production_regex_baseline.csv`, `survey_production_v2.csv`, `longtail_tasks.md`, `replay/`, `macro.ijm`, `INPUTS.json`, `run_replay.py`, `MATCH_REPORT.md`, `docs/rpc-examples/`, `README.md` file reference is preserved; every `runMacro`, `takeScreenshot`, `getRoisAsGeoJson`, `executeJavaScript`, `convertToMcpUrl` method name is preserved; every §§2/4/7/8/3 cross-reference is preserved; every ACQUIRE / EXECUTE / MATCH axis name is preserved; MIT licence, Zenodo archive, and Hypha workspace-token authentication model are preserved. Claim-diff against v0.1 empty, `placeholder-value` inventory unchanged at 185 spans (nineteenth consecutive empty-claim-diff iteration). This closes the last main-body biologist-voice gap — the complete argumentative spine Key Points → Abstract v0.6 → §1 v0.2 → Box 1 → §2 v0.2 → Fig 1 → Box 2 → §3 v0.2 → Fig 2 → §4 v0.2 → Fig 3 → §§5–7 → Fig 4–6 → §8 v0.2 → Fig 7 → §9 v0.2 → **§10 v0.2** → Cover letter v0.2 → Research Briefing v0.2 now reads in publication-ready biologist voice end-to-end (pending §§5/6/7 Gate G partner landings). Online Methods remains in editorial voice by genuine genre constraint — it is a deliverable addressed to a copy-editor with fixed subsection headings and pre-registered protocol language, not an argument addressed to the biologist.*

### How to open the tool and find the source

You can open ImageJ.JS in any recent browser at `https://ij.aicell.io`. The page loads as a single HTML document and a WebAssembly payload; there is nothing to install, and nothing runs on our servers. The source code lives on GitHub at `https://github.com/aicell-lab/imagej.js`; the repository holds the browser harness (`index.html`), the Hypha-RPC service layer (`hypha-imagej-service.js`), the CheerpJ virtual-file-system mount and plugin-loading machinery, the collaboration driver/observer implementation (`collab/`), and the build tooling we use to freeze a reproducible runtime. We release it under the MIT licence, which is permissive and compatible with the Fiji and CheerpJ upstream licences it inherits. The version of record for this paper is the git tag `v1.0-paper`, archived on Zenodo at DOI [DOI]; every URL, every macro, and every analysis cited in the paper resolves against this pinned tag, so a reader who re-opens a shared URL well after publication sees the tool exactly as it existed at submission. Continued development happens on the `main` branch — use the archived tag when you want to reproduce a result as it appears in the paper, and `main` when you want the current state of the code.

### What data sits under the paper's numbers

Three kinds of data sit under the empirical claims of §§2 and 4, and we ship every one of them so you can audit any number in the paper yourself. The survey corpus contains the 200 open-access microscopy papers we sampled for §2, together with the extraction schema (`survey_schema.md`), the regex-baseline pass (`survey_production_regex_baseline.csv`), the LLM v2 upgraded pass (`survey_production_v2.csv`), and per-claim provenance records so that each of the [48]%, [48]%, and [20]% headline figures is traceable to a specific row and extractor version. The long-tail foundation-model benchmark is released as `longtail_tasks.md` (the 30-task specification with inclusion criteria), together with the task imagery (where licence permits — otherwise as a citation), the human-expert ImageJ.JS macros, and the evaluation harness that reproduces the [X] mean IoU and [Y]/30 vs [Z]/30 headline numbers. The deterministic replay corpus behind §4 lives in the `replay/` directory; each re-run published analysis sits in its own subdirectory that holds the source Fiji macro (`macro.ijm`), the input bundle descriptor (`INPUTS.json`), the replay script (`run_replay.py`), the ImageJ.JS outputs, and a written `MATCH_REPORT.md` that records the ACQUIRE / EXECUTE / MATCH axes separately. The Week-1 three-candidate pilot is archived verbatim; the full [N]-analysis corpus replaces it in the same layout at revision time.

### How to call the tool from a script or an agent

If you want to call ImageJ.JS from a script, a Python notebook, or an AI agent rather than from the browser, the interface is the Hypha-RPC service defined in `hypha-imagej-service.js` and, by URL rewrite, the Model Context Protocol endpoint described in §8. Four methods are exposed: `runMacro` executes a Fiji macro string against the current session; `takeScreenshot` returns a rendered frame of the image canvas; `getRoisAsGeoJson` exports the active ROI set as GeoJSON features with per-ROI provenance; and `executeJavaScript` evaluates arbitrary JavaScript against the in-page CheerpJ JVM. The MCP endpoint is derived from the Hypha service URL by the published `convertToMcpUrl` helper (`hypha-imagej-service.js:880`). We distribute a minimal example notebook showing each method called from an external Python caller, and a second notebook showing the same methods addressed as MCP tools, both under `docs/rpc-examples/`. The authentication model is Hypha's workspace-token mechanism; this paper introduces no separate authorisation layer.

### What happens with your data — and what we record
